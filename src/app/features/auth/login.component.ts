...
import { passwordLengthValidators } from '../../core/validation/auth-validation';
...
@Component({
      ...
      <div class="auth-card card">
        ...
        <form [formGroup]="loginForm" (ngSubmit)="onSubmit()">
          ...
          <div class="form-group mb-5">
            ...
            <small class="field-error" *ngIf="shouldShowPasswordError()">Password must be 8-72 characters.</small>
          </div>
          ...
          <div class="google-login-wrap">
            <div class="google-button-shell">
              ...
              <div id="google-signin-button" class="google-signin-slot" [class.hidden-google-slot]="googleUnavailable"></div>
            </div>
            <button
  ...
  styles: [`
    ...
    .forgot-link { font-size: 0.7rem; color: var(--primary); text-decoration: none; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; }
...
export class LoginComponent implements OnInit {
  ...
  constructor(
    ...
    this.loginForm = this.fb.group({
      ...
      password: ['', passwordLengthValidators()]
    });
  }
  ...
  private waitForGoogleAndInitialize(attempt: number = 0): void {
    ...
    if (attempt >= 20) {
      ...
      console.error('[LoginComponent] Google Identity Services failed to load');
      return;
    }
  ...
  private handleGoogleCredential(response: GoogleCredentialResponse): void {
    if (!response.credential) {
      console.error('[LoginComponent] Google credential response did not include an ID token');
      return;
    }
